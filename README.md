...
```python
renderer = sdk.PluginTakumi.create_renderer(cache_max_bytes=64 * 1024 * 1024)
...
```python
renderer = sdk.PluginTakumi.create_renderer(
    load_default_fonts=True,
    fonts=[custom_font],
)
```

独立实例不会经过模块的接口代理，因此调用渲染或测量方法时，如需统一的内置字体回退栈，应显式传入 `font_families=sdk.PluginTakumi.families`。
